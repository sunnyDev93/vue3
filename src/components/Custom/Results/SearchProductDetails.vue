...
const components: Ref<{ CRITERIA_NAME: string; CRITERIA_VALUE: string }[]> =
  ref([]);
const documents: Ref<{ ART_MEDIA_TYPE: string; ART_MEDIA_SOURCE: string }[]> =
  ref([]);
const specifications: Ref<any[]> = ref([]);
const loading: Ref<boolean> = ref(false);
const imageUrl: Ref<string> = ref(
  `${S3_STORAGE_IMAGE}articles/article_img_not_found.png`
);
...
const eancode = computed(() => {
  return articleData.value.EAN_NUMBERS
    ? articleData.value.EAN_NUMBERS.split(" ")
    : "";
});
</script>
